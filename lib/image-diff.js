...
ImageDiff.createDiff = function (options, cb) {
  ...
  exec(diffCmd, function processDiffOutput (err, stdout, stderr) {
    ...
    return cb(null, percentDifference <= threshold,percentDifference);
  });
};
ImageDiff.prototype = {
    ...
    async.waterfall([
      ...
      function createDiff (cb) {
        ImageDiff.createDiff({
          ...
          diffPath: diffPath,
          threshold: threshold
        }, function saveResult (err, _imagesAreSame,_percentDifference) {
          imagesAreSame = _imagesAreSame;
          percentdiff = _percentDifference;
          cb(err);
        });
      }
      ...
      async.forEach(cleanupPaths, function cleanupFile (filepath, cb) {
        ...
        callback(err, imagesAreSame,percentdiff);
      });
    });
  }
};
